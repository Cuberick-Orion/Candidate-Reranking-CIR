...
def stage1_training_fiq(train: bool,
        ...
        blip_learning_rate: float, blip_max_epoch:int, blip_img_tune: bool, batch_size: int,
        blip_bs: int, validation_frequency: int, preprocess_val: bool, 
        transform: str, save_training: bool, save_best: bool, **kwargs):
    """
    Train the stage I model
    """
    ...
    if not blip_img_tune and preprocess_val:
        ...
        index_features_list = []
        index_features_normed_pooled_list = []
        index_names_list = []
        ...
        for idx, dress_type in enumerate(val_dress_types):
            ...
            index_features_and_names = extract_index_features(classic_val_dataset, model, blip_stage1=True)
            index_features_list.append(index_features_and_names[0])
            index_features_normed_pooled_list.append(index_features_and_names[1])
            index_names_list.append(index_features_and_names[2])
            torch.cuda.empty_cache() # at the end approx. 32G VRAM
        ...
        if train:
            ...
            with experiment.train():
                ...
                for idx, (reference_images, target_images, captions,) in enumerate(train_bar):  # Load a batch of triplets
                    ...
                    if not blip_img_tune:
                        with torch.no_grad():
                            ...
                            target_image_features = torch.vstack(
                                [model.img_embed(mini_batch, return_pool_and_normalized=True)[-1].float() for mini_batch in target_images_list])
                    else:
                    ...
                    with torch.cuda.amp.autocast():
                        logits = model.img_txt_fusion(reference_image_features, target_image_features, input_captions, train=True)
                        ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                with torch.no_grad():
                    ...
                    # FashionIQ category)
                    for relative_val_dataset, index_features, index_features_normed_pooled, index_names, idx in zip(relative_val_datasets,
                                                                                    index_features_list,
                                                                                    index_features_normed_pooled_list, 
                                                                                    index_names_list,
                                                                                    idx_to_dress_mapping):
                        recall_at10, recall_at50 = compute_fiq_val_metrics(relative_val_dataset, model, 
                                                                           index_features, index_features_normed_pooled,
                                                                           index_names)
                ...
                if not preprocess_val: # manually free up VRAM
                    del index_features_list
                    del index_features_normed_pooled_list
                    torch.cuda.empty_cache()

                # Save model
                if save_training and train: # don't save ckpt if only validating
...
def stage1_training_cirr(train: bool,
        num_workers: int, num_epochs: int,
        blip_learning_rate: float, blip_max_epoch:int, blip_img_tune: bool, batch_size: int,
        blip_bs: int, validation_frequency: int, preprocess_val: bool,
        transform: str, save_training: bool, save_best: bool, **kwargs):
    ...
    if not blip_img_tune and preprocess_val:
        ...
        val_index_features, val_index_features_p, val_index_names = extract_index_features(classic_val_dataset, model, blip_stage1=True)
        torch.cuda.empty_cache()
        ...
        if train:
            ...
            with experiment.train():
                ...
                for idx, (reference_images, target_images, captions,) in enumerate(train_bar):  # Load a batch of triplets
                    ...
                    with torch.cuda.amp.autocast():
                        logits = model.img_txt_fusion(reference_features, target_features, captions, train=True)
                        ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                        loss_raw = crossentropy_criterion(logits, ground_truth)
        ...
        if epoch % validation_frequency == 0:
            ...
            with experiment.validate():
                with torch.no_grad():
                    ...
                    results = compute_cirr_val_metrics(relative_val_dataset, model, val_index_features, val_index_features_p,
                                                    val_index_names)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--num-epochs", default=20, type=int, help="number training epochs")
    ...
    parser.add_argument("--batch-size", default=1024, type=int, help="Batch size of the stageI training")
